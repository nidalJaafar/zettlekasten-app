...
type SqliteParams = NonNullable<Parameters<Awaited<ReturnType<typeof initSqlJs>>['Database']['prototype']['exec']>[1]>
...
export async function createTestDb(): Promise<Database> {
  ...
  return {
    async execute(sql: string, params: unknown[] = []) {
      ...
      stmt.run(params as SqliteParams)
      stmt.free()
    },
    async query<T>(sql: string, params: unknown[] = []) {
      const result = sqlite.exec(sql, params as SqliteParams)
